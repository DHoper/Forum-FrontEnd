  ...
  >
    <div class="bg-white w-full py-8 px-4">
      ...
      >
        <button
          @click="router.back()"
          class="self-start text-sm xl:text-xl font-bold text-stone-800"
        >
          上一頁
        </button>
        <div class="mx-auto border-2 border-stone-800 w-full px-24 py-16">
          <div class="flex items-center gap-4">
            ...
            <div class="flex gap-2 items-baseline justify-start flex-1">
              <span class="text-lg xl:text-xl text-stone-700 font-bold">{{
                authorData.username
              }}</span>
          ...
          <h1
            class="my-4 text-2xl xl:text-4xl xl:pt-4 text-stone-900 font-bold"
          >
            {{ postData.title }}
          </h1>
          <div class="flex gap-2 flex-wrap">
            <span
              v-for="tag in postData.topicTags"
          ...
          <p class="mt-10 text-stone-700 xl:text-lg whitespace-pre-wrap">
            {{ postData.content }}
          </p>
          <div
            v-if="postData.images && postData.images.length > 0"
          ...
          <div class="flex justify-between items-center">
            ...
            >
              <HeartIcon
                @click="handleLikes"
                class="w-4 xl:w-6 group-focus:scale-[115%] transition-all duration-300"
              />{{ postData.likes }}
            </span>
          </div>
        </div>
        ...
        >
          <span class="font-bold text-stone-600 tracking-widest xl:text-xl"
            >留言區</span
          >
          <div
            class="border-b-[.0938rem] xl:border-b-2 border-stone-300 my-4"
          ></div>
          <form method="POST" @submit.prevent="handleCommentSubmit">
            <div
              class="mt-10 mb-4 border-2 border-stone-600 p-4 xl:p-8 xl:text-lg"
            >
              <textarea
                v-model="commentPost"
                name=""
            ...
            <div class="flex justify-end gap-4 xl:gap-6">
              <button
                type="button"
                @click="commentPost = ''"
                class="text-green-400 px-5 py-2 xl:text-lg hover:text-green-600"
              >
                取消
              </button>
              <button
                ...
                type="submit"
                class="text-white bg-gray-300 px-5 py-2 xl:text-lg transition-all"
              >
                送出
              </button>
            </div>
              ...
              <div v-if="commentAuthorData[index]">
                <div class="flex items-center gap-4">
                  ...
                  <div class="flex gap-2 items-baseline justify-start flex-1">
                    ...
                    <span
                      class="text-sm xl:text-base text-stone-500 text-center"
                ...
                <div>
                  <p
                    class="mt-10 text-stone-700 xl:text-lg whitespace-pre-wrap overflow-auto"
                  >
                    {{ comment.content }}
                  </p>
                </div>
