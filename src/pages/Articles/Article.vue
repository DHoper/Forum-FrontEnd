...
<template>
  <div class="bg-articles bg-no-repeat bg-cover bg-center w-full bg-fixed">
    ...
    <div class="bg-white w-full mt-[90vh] py-12 px-4">
      <div
        class="mx-auto flex flex-col bg-white w-[62rem] translate-y-[-7rem] pt-28 px-5"
      >
        <button
          @click="router.back()"
          class="self-start text-sm font-bold text-stone-800"
        >
          上一頁
        </button>
        <div class="mt-12 border-2 border-stone-800 px-24 py-16 mb-20">
          ...
          <div class="flex justify-between">
            ...
            <span
              class="group flex gap-1 hover:cursor-pointer focus:text-red-500 transition-all"
              tabindex="0"
            >
              <HeartIcon
                class="w-4 group-focus:scale-[125%] transition-all duration-300"
              />{{ article.stats.likes }}
            </span>
          </div>
        </div>
