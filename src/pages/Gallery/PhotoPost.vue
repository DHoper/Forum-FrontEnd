...
import { postComment, deleteComment } from '../../api/photoPost/photoPostComment.ts';
import { useUserStore } from '../../store/user.ts';
import { formatDateTime } from '../../utils/formattingUtils.ts';
import { CommentType, DialogType, UserDataType, PhotoPostType, AuthorDataType } from '../../types.ts';
...
<template>
    <div v-if="postData && postAuthorData" class="z-10 flex absolute top-0 left-0 w-full h-full  overflow-hidden">
        <div ref="leftBlock"
            class="animate-slideInLeft basis-[30%] bg-white py-10 px-8 overflow-auto">
            <span class="font-bold text-stone-600 xl:text-xl tracking-widest">留言區</span>
            ...
            <form method="POST" @submit.prevent="handleCommentSubmit">
                ...
                <div class="flex justify-end gap-2 xl:my-8">
                    <button type="button" @click="commentPost = ''"
                        class="text-green-400 px-5 py-2 xl:px-6 xl:text-lg hover:text-green-600">取消</button>
                    <button ref="commentSubmitButton" type="submit"
                        class="text-white bg-gray-300 px-5 py-2 xl:px-6 xl:text-lg transition-all ">送出</button>
                </div>
            </form>
            <ul v-if="commentsData.length > 0 && commentAuthorData.length > 0" class="mt-8 xl:mt-16">
                    ...
                    <div v-if="commentAuthorData[index]">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex gap-2 items-baseline justify-start flex-1">
                                <span class="text-sm xl:text-xl text-stone-800 font-bold">{{ commentAuthorData[index].username
                                }}</span>
                                <span class="text-xs xl:text-base text-stone-500 text-center">{{ formatDateTime(comment.createdAt!,
                                    'concise')
                                }}</span>
                        ...
                        <div>
                            <p class="mt-4 xl:my-8 text-sm xl:text-xl text-stone-700">{{ comment.content }}</p>
                        </div>
                        <div class="border-b-[.0938rem] xl:border-b-2 border-gray-300 my-4"></div>
                    </div>
                </li>
            </ul>
            <div v-else class="border-b-[.0938rem] xl:border-b-2 border-gray-300 mt-10 xl:mt-20 text-center text-gray-400 py-1">尚未有留言</div>
        </div>
        <div ref="rightBlock"
                ...
                <div
                    ...
                    <Transition :duration="1000">
                        <div v-if="!turnPage" class="h-full flex flex-col">
                            <div class="flex gap-4">
                                ...
                                <div
                                    ...
                                    <h2 class="text-2xl font-bold text-center xl:text-3xl">{{ postData.title }}</h2>
                                    <p class="text-sm text-center text-stone-500 py-2 xl:py-4 xl:text-xl">{{
                                        postData.location }}</p>
                                    <p
                            ...
                            <div class="flex gap-4 justify-between italic xl:mt-4">
                                <div class="flex items-center py-2 gap-2">
                                    ...
                                    <div class="flex items-baseline">
                                        <p class="text-stone-600 xl:text-xl xl:text-stone-700 font-bold">{{
                                            postAuthorData.username! }}</p>
                                        <p v-if="postData.createdAt" class="text-stone-600 xl:text-base text-sm">
                                            -- {{ formatDateTime(postData.createdAt) }}
                                        </p>
                                        <p v-if="!postData.isEdit" class="text-stone-600 xl:text-base text-sm">發布</p>
                                        <p v-else class="text-stone-600 xl:text-base text-sm">已編輯</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-4">
                                    <span class="flex items-center gap-1 xl:text-lg text-blue-900">
                                        <EyeIcon class="w-6 xl:w-8" />
                                        <span>{{ postData.views }}</span>
                                    </span>
                                    <span class="flex items-center gap-1 xl:text-lg group"
                        ...
                        <div v-else class="xl:w-[calc(100%-2.5rem)] h-full absolute top-0">
                            <div class="flex justify-around xl:pr-10 items-center h-full p-4 xl:py-8 xl:px-0">
                                <div
                                ...
                                <div class="bigImg border-4 border-b-2 border-stone-600 rounded-sm">
                                    <img :src="postData.images[pictureIndex].url"
...
</template>../../api/photoPostComment.ts../../api/photoPostComment.ts../../api/comment/photoPostComment.ts../../api/comment/photoPostComment.ts../../api/photoPost/photoPostComment.ts../../api/photoPost/photoPostComment.ts../../api/user/user.ts