            ...
            <div class="basis-[70%] xl:basis-[80%] h-full">
                <div ref="rightBlock" class="h-full flex flex-wrap overflow-y-auto overflow-x-hidden">
                    <div v-for="galleryData in galleryDataset"
                        ...
                        <div @click="openPhoto(galleryData._id!)"
                            ...
                            <div v-if="galleryData.createdAt">
                                <p class="text-xs xl:text-base text-[#fdfdfdda]">
                                    {{ formatDateTime(galleryData.createdAt) }}
                                </p>
                                <h3 class="text-xl xl:text-3xl  my-2 text-[#fdfdfdda]">
                                    {{ galleryData.title }}
                                </h3>
                                <p class="text-xs xl:text-base text-[#fdfdfdda]">
                            ...
                            <div>
                                <div class="border xl:mb-2 border-[#fdfdfdb7]"></div>
                                <div class="flex items-center justify-between h-7">
                                    <span class="button text-sm xl:text-base tracking-widest">查看更多</span>
                    ...
                    <router-link class="flex-grow" v-if="addBlock === 'block'" :to="{ name: 'CreatePost' }">
                        <a
                            class="w-full h-full py-8 xl:py-10 flex justify-center items-center text-xl xl:text-3xl font-bold bg-gray-100 text-stone-700 cursor-pointer hover:text-stone-800 hover:scale-105 hover:bg-white hover:z-10 transition-all duration-[1150ms]">
                            添加更多發現
                        </a>
                    </router-link>
                    <router-link v-else :to="{ name: 'CreatePost' }">
                        <PlusCircleIcon
                            class="fixed bottom-12 right-12 w-16 h-16 xl:w-20 xl:h-20 z-10 text-white cursor-pointer hover:scale-110 hover:text-green-400 transition-all ease-in" />
                    </router-link>
                </div>
            </div>
